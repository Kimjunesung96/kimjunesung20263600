...
class MySQLHelper:
    ...
    def search_by_date(self, keyword):
        query = 'SELECT * FROM mars_weather WHERE mars_date LIKE %s'
        return self.fetch_all(query, (f'%{keyword}%',))

    def search_by_temp_range(self, min_temp, max_temp):
        query = (
            'SELECT * FROM mars_weather '
            'WHERE temp BETWEEN %s AND %s'
        )
        return self.fetch_all(query, (min_temp, max_temp))

    def search_by_storm(self, storm):
        query = 'SELECT * FROM mars_weather WHERE storm = %s'
        return self.fetch_all(query, (storm,))
...
def main():
    ...
    row_count = db_helper.count_rows()
